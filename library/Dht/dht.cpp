...
{
    // READ VALUES 10 times to filter out wrong values
	int rv = DHTLIB_ERROR_TIMEOUT;
    for (int i=0; i<10; i++){
        rv = read(pin);
        if (rv == DHTLIB_OK) {
			break;
    ...
    // CONVERT AND STORE
	int isNegtive = bits[3] >> 7;
	if (isNegtive)
		isNegtive = -1;
	else
		isNegtive = 1;
	
    humidity    = bits[0] + float(bits[1] & 0x8F) / 10;  // bits[1] == 0;
    temperature = isNegtive * (bits[2] + float(bits[3] & 0x8F) / 10);  // bits[3] == 0;

    // TEST CHECKSUM
    // bits[1] && bits[3] both 0
    uint8_t sum = bits[0] + bits[1] + bits[3] +bits[2];
